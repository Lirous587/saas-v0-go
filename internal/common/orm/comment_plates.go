...
func (o *CommentPlate) GetPlateComments() CommentSlice {
	...
	return o.R.GetPlateComments()
}

func (r *commentPlateR) GetPlateComments() CommentSlice {
	if r == nil {
		return nil
	}

	return r.PlateComments
}
...
var (
	commentPlateAllColumns            = []string{"id", "belong_key", "tenant_id", "summary"}
	commentPlateColumnsWithoutDefault = []string{"belong_key", "tenant_id", "summary"}
...
func (o *CommentPlate) PlateComments(mods ...qm.QueryMod) commentQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"comments\".\"plate_id\"=?", o.ID),
	)
...
// LoadPlateComments allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (commentPlateL) LoadPlateComments(e boil.Executor, singular bool, maybeCommentPlate interface{}, mods queries.Applicator) error {
	var slice []*CommentPlate
	var object *CommentPlate
	...
	query := NewQuery(
		qm.From(`comments`),
		qm.WhereIn(`comments.plate_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.PlateComments = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &commentR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.PlateID {
				local.R.PlateComments = append(local.R.PlateComments, foreign)
				if foreign.R == nil {
					foreign.R = &commentR{}
				}
				foreign.R.Plate = local
...
// AddPlateCommentsG adds the given related objects to the existing relationships
// of the comment_plate, optionally inserting them as new records.
// Appends related to o.R.PlateComments.
// Sets related.R.Plate appropriately.
// Uses the global database handle.
func (o *CommentPlate) AddPlateCommentsG(insert bool, related ...*Comment) error {
	return o.AddPlateComments(boil.GetDB(), insert, related...)
}

// AddPlateComments adds the given related objects to the existing relationships
// of the comment_plate, optionally inserting them as new records.
// Appends related to o.R.PlateComments.
// Sets related.R.Plate appropriately.
func (o *CommentPlate) AddPlateComments(exec boil.Executor, insert bool, related ...*Comment) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.PlateID = o.ID
	...
	if o.R == nil {
		o.R = &commentPlateR{
			PlateComments: related,
		}
	} else {
		o.R.PlateComments = append(o.R.PlateComments, related...)
	}
